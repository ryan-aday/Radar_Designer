...
st.caption("Variables: pulse width $\tau$, bandwidth $BW$, wavelength $\lambda$, pulse repetition frequency PRF, beamwidth $\theta$, antenna rotation RPM.")

col1, col2, col3 = st.columns(3)
with col1:
    pw_us = st.number_input("Pulse width $\tau$ (µs)", min_value=0.01, value=1.0)
    prf_hz = st.number_input("PRF (Hz)", min_value=1.0, value=1000.0)
with col2:
    bandwidth_mhz = st.number_input("Transmit bandwidth $BW$ (MHz)", min_value=0.001, value=5.0, key="mode_bw")
    freq_ghz = st.number_input("Carrier $f$ (GHz)", min_value=0.1, value=10.0, key="mode_f")
with col3:
    beamwidth_deg = st.number_input("Beamwidth $\theta$ for dwell (°)", min_value=0.1, value=2.0)
...
st.caption("Dwell ≈ 6·PRT/beam for MTI; use staggered PRFs to balance range/velocity ambiguities. Duty cycle = τ/PRI per eGuide.")
