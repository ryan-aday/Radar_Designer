...
st.caption(
    "Variables: $ERP_J$ jammer EIRP (W), $ERP_T$ target/comm EIRP (W), $G_M$ mainlobe gain, $G_S$ sidelobe gain, $R_T$ range to target (km), $R_J$ range to jammer (km), $f$ MHz carrier."
)

col1, col2, col3 = st.columns(3)
with col1:
    erp_j = st.number_input("Jammer ERP $ERP_J$ (W)", min_value=0.1, value=10000.0)
    erp_t = st.number_input("Target ERP $ERP_T$ (W)", min_value=0.001, value=50.0)
    freq_mhz = st.number_input("Carrier $f$ (MHz)", min_value=1.0, value=3000.0)
with col2:
    gm = st.number_input("Radar mainlobe gain $G_M$ (dBi)", value=30.0)
    gs = st.number_input("Radar sidelobe gain $G_S$ (dBi)", value=0.0)
with col3:
    rt = st.number_input("Range radar→target $R_T$ (km)", min_value=0.1, value=100.0)
    rj = st.number_input("Range radar→jammer $R_J$ (km)", min_value=0.1, value=50.0)
