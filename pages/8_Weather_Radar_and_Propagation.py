...
st.markdown(
    ...
    **Atmospheric attenuation:** use rain + gaseous loss per km when above 10 GHz; eGuide Fig. 10 shows peaks near oxygen and water-vapor lines.
    """
)
...
with col2:
    rain_att_db_km = st.number_input("Rain/gas attenuation (dB/km)", min_value=0.0, value=0.12, help="Use eGuide Fig. 10 per km.")
...
st.subheader("Radar horizon and ducting notes")
...
st.markdown(
    r"""
    Horizon range follows the eGuide approximation \(R_{LOS} \approx 4.12(\sqrt{h_{ant}} + \sqrt{h_{tgt}})\) with heights in meters.
    Height estimate uses \(H \approx R\sin e + \frac{R^2}{2r_{equiv}}\) with \(r_{equiv}\approx 8500\,\text{km}\) from the refraction section.
    Ducting (surface or evaporation ducts) can trap waves beyond line of sight; elevated ducts form at density boundaries.
    Standard/negative refractivity gradients define normal, sub-refraction, super-refraction, and ducting cases per the eGuide charts.
    """
)

with st.expander("Paste weather figures"):
    st.write("Rain attenuation curves, refractivity gradient sketches, and ducting diagrams from the eGuide fit here.")
